...
from src.optimize.models.RSA_PATH_CHANNEL.params import Parameter, Width
...
from utils.network import calc_path_weight, is_edge_in_path, select_modulation_format
...
def make_input_lower(params: Parameter) -> PathLowerBoundInput:
    ...
    # load all paths
    with open(params.paths_dir, mode="rb") as f:
    ...
    input       = PathLowerBoundInput(E=E, S=S, D=D, P=P, num_slots=num_slots, 
                                      delta=delta, result_dir=params.result_dir, 
    ...
    for d_ind, demand in D.items():
        for p_ind, path in enumerate(P[d_ind]):
            ...
            required_slots = calc_required_slots(demand[2], modulation_format, 
                                                  params.width, params.TRAFFIC_BPSK)
