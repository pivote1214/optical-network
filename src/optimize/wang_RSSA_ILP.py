    ...
    for s in V:
        for d in V:
            if s != d:
                ...
                for physical_path in Sorted_All_physical_path_sd[:3]:
...
#             sht1.write(j,3,R[j][2])
        # xls.save('C:\\Users\Kanoh\Desktop\SDM models\Requests_for_SDM\\'+'request_data_'+str(R_num)+'_'+str(iteration_num)+'.xls')

# def Request_reader(R_num, iteration_num):
#     workbook = xlrd.open_workbook('C:\\Users\Kanoh\Desktop\SDM models\Requests_for_SDM\\request_data_'+str(R_num)+'_'+str(iteration_num)+'.xls')
...
def Request_reader(R_num, seed):
    import pickle
    file_name = f'/Users/pivote1214/Documents/Codes/OpticalNetwork/data/input/demands/NSFNET/demands_{seed}_num={R_num}_p=[50, 100, 150, 200].pickle'
    with open(file_name, 'rb') as f:
        R = pickle.load(f)
    for r_ind, r in R.items():
        R[r_ind] = (r[0], r[1], r[2])
...
workbook.save('./SDM_result'+str(R_num)+'.xls')
worksheet_index = 0
...
for s_i in s_i_set:
    ...
    #    for R in R_set:
        R = Request_reader(R_num, seed)
    #    R =  R_set[5]
        '''Pre-calculation'''
        AP, M, P, F_rp, O_rp = Simulation_initialization()
        Best_PEG, LBound_ILP_SLC_RMS, Solution_ILP_SLC_RMS, undone_SLC_RMS = ILP_SLC_Relax(Time_Limit_SLC_RMS)
        ...
        workbook = xlrd.open_workbook('./SDM_result'+str(R_num)+'.xls')
        ...
        new_workbook.save('./SDM_result'+str(R_num)+'.xls')
    worksheet_index += 1
